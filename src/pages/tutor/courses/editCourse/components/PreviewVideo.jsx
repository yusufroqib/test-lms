...
const formSchema = z.object({
	previewVideoUrl: z.string().min(1),
});
export const PreviewVideoForm = ({ initialData, courseId }) => {
	...
	const uploadVideo = async (file, inputs) => {
		...
		if (existingPreviewVideoUrl) {
			const existingVideoRef = ref(storage, existingPreviewVideoUrl);
			try {
		...
		<div className="mt-6 border bg-slate-100 rounded-md p-4">
			<div className="font-medium flex items-center justify-between">
				...
				<Button onClick={toggleEdit} variant="ghost">
					{isEditing && <>Cancel</>}
					{!isEditing && !initialData.previewVideoUrl && (
						<>
			...
			{!isEditing &&
				(!initialData.previewVideoUrl ? (
					...
					<div className="relative aspect-video mt-2">
						...
						<video
							src={initialData.previewVideoUrl}
							controls
			...
			{isEditing && (
				<div>
					...
					<div className="text-xs text-muted-foreground mt-4">
						Upload this course&apos;s preview/demo video
					</div>
			...
			{initialData.previewVideoUrl && !isEditing && (
