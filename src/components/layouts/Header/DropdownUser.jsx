...
const DropdownUser = () => {
	...
	// console.log(stripeOnboardingComplete)
	...
	return (
		<>
			<Overlay isOpen={dropdownOpen} onClose={() => setDropdownOpen(false)} />
			<div className="relative">
				...
				<BecomeTutorDialog
					open={openBecomeTutor}
					setOpen={setOpenBecomeTutor}
				/>
				<Link
					ref={trigger}
					...
					<ul className="flex flex-col gap-5 border-b border-stroke px-6 py-7.5 dark:border-strokedark">
						...
						{status === "Tutor" && !stripeOnboardingComplete && (
							<li>
								<Link
									onClick={() => setDropdownOpen(false)}
									to="/tutors/stripe-connect/refresh"
									className="flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-blue-500 lg:text-base"
								>
									<LinkIcon /> Link Account to Stripe
								</Link>
							</li>
						)}
						{status === "Tutor" && (
						...
						{/* <li>
            <Link to="/pages/settings" className="flex items-center gap-3.5 text-sm font-medium duration-300 ease-in-out hover:text-blue-500 lg:text-base">
              ...
              Account Settings
					...
					<button
						...
						className="flex items-center gap-3.5 px-6 py-4 text-sm font-medium duration-300 ease-in-out text-red-500 hover:text-red-700 lg:text-base"
					>
						<svg
							className="fill-current"
							width="22"
